...
export class FormSubmissionsService {
  ...
  async create(dto: CreateFormSubmissionDto): Promise<FormSubmissionEntity> {
    ...
    const overall = template ? computeOverallScore(template.definition, sections) : { percentage: null, mention: null };
  ...
  async findOne(id: string): Promise<FormSubmissionEntity> {
    const submission = await this.repository.findOne({ where: { id }, relations: ['etablissement'] });
    if (!submission) {
   ...
   */
  async findOneScoped(id: string, user: UserEntity): Promise<FormSubmissionEntity> {
    const submission = await this.findOne(id);
    if (!this.isInScope(submission, user)) {
      throw new ForbiddenException("Ce formulaire n'est pas dans votre périmètre.");
    }
    return submission;
  }
   ...
   */
  findAllScopedForUser(user: UserEntity, query: QueryFormSubmissionDto): Promise<FormSubmissionEntity[]> {
    const qb = this.scopedQueryBuilder(user);

    if (query.formCode) {
      qb.andWhere('submission.formCode = :formCode', { formCode: query.formCode });
    }
    ...
    if (query.inspecteurId) {
      qb.andWhere('submission.inspecteurId = :filterInspecteurId', { filterInspecteurId: query.inspecteurId });
    }
    if (query.status) {
      qb.andWhere('submission.status = :status', { status: query.status });
    }
    if (query.schoolYear) {
      qb.andWhere('submission.schoolYear = :schoolYear', { schoolYear: query.schoolYear });
    }
    if (query.dateFrom) {
      qb.andWhere('submission.createdAt >= :dateFrom', { dateFrom: query.dateFrom });
    }
    if (query.dateTo) {
  ...
  async stats(user: UserEntity): Promise<DashboardStats> {
    ...
    const byFormCodeRaw = await base
      ...
      .getRawMany<{ formCode: string; count: string; averagePercentage: string | null }>();

    const byFormCode: FormCodeStats[] = byFormCodeRaw.map((row) => ({
      formCode: row.formCode as FormCodeStats['formCode'],
      count: Number(row.count),
      averagePercentage: row.averagePercentage !== null ? Number(row.averagePercentage) : null,
    }));

    const totalInspections = byFormCode.reduce((sum, row) => sum + row.count, 0);
    ...
    const latest = await base.clone().orderBy('submission.createdAt', 'DESC').limit(10).getMany();
   ...
   */
  async updateStatus(id: string, status: FormSubmissionStatus): Promise<FormSubmissionEntity> {
    const submission = await this.findOne(id);
  ...
  private scopedQueryBuilder(user: UserEntity): SelectQueryBuilder<FormSubmissionEntity> {
   ...
   */
  private applyScope(qb: SelectQueryBuilder<FormSubmissionEntity>, user: UserEntity): void {
    switch (user.role) {
      case 'super_admin':
        return;
      case 'ige_admin':
        if (user.zone) {
          qb.andWhere('etablissement.zone = :userZone', { userZone: user.zone });
        }
        return;
      ...
      case 'inspecteur':
        ...
        qb.andWhere('submission.inspecteurId = :scopeInspecteurId', { scopeInspecteurId: user.inspecteurId });
        return;
      case 'enseignant':
        ...
        qb.andWhere('submission.enseignantId = :scopeEnseignantId', { scopeEnseignantId: user.enseignantId });
        return;
    }
  }

  private isInScope(submission: FormSubmissionEntity, user: UserEntity): boolean {
    switch (user.role) {
      case 'super_admin':
        return true;
      case 'ige_admin':
      ...
      case 'chef_etablissement':
        return Boolean(user.etablissementId) && submission.etablissementId === user.etablissementId;
      case 'inspecteur':
        return Boolean(user.inspecteurId) && submission.inspecteurId === user.inspecteurId;
      case 'enseignant':
        return Boolean(user.enseignantId) && submission.enseignantId === user.enseignantId;
    }
  }
}
